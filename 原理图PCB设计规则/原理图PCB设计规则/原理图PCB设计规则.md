...
## 1. 电源的输入输出需要看效率，同一个电源芯片在不同的电流下效率不同（效率=输出的功率/输入的功率)    



## 2. I2C需要进行上拉,上拉的电阻可以看数据上,一般是4.7k   

## 3.使能信号尽量不要取中间值,一般直接0V低,最高值高,别整个0.6V之类的当做低   

## 4.进行硬件修改的时候,尽量保持主控或单片机的引脚功能不变,可以减少软件的工作量,加快项目进度    

## 5. 进行硬件修改的时候,连线需要考虑PCB绘制的难度,比如电源芯片1拖2,按顺序来连接,不要出现交叉的现象
